...
int main(){
    ...
    while(N != 0){
        ...
        set<string> cameras;
        vector<int> cameint = G.getArticulationPoints();
        for(int i = 0; i < cameint.size(); i ++){
            string s = invNames[cameint[i]];
            cameras.insert(s);
        }

        cout << "City map #" << cas << ": "<<cameras.size() << " camera(s) found" << endl;
        for(set<string>::iterator it = cameras.begin(); it != cameras.end(); ++it){
            cout << *it << endl;
        }
        cin >> N;
    }
